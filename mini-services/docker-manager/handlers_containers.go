...
func handleListImages(w http.ResponseWriter, r *http.Request) {
	images, err := execDockerImages()
	if err != nil {
		log.Printf("Warning: could not list images: %v", err)
		respondJSON(w, http.StatusOK, []ImageInfo{})
		return
	}
...
func handleListServices(w http.ResponseWriter, r *http.Request) {
	services, err := execDockerComposeServices()
	if err != nil {
		log.Printf("Warning: could not list services: %v", err)
		respondJSON(w, http.StatusOK, []ServiceInfo{})
		return
	}
...
func registerContainerRoutes(r *mux.Router) {
	...
	images := r.PathPrefix("/api/images").Subrouter()
	images.Use(authMiddleware)
	images.HandleFunc("", handleListImages).Methods("GET")

	// Services (Docker Compose projects)
	services := r.PathPrefix("/api/services").Subrouter()
	services.Use(authMiddleware)
	services.HandleFunc("", handleListServices).Methods("GET")
}
