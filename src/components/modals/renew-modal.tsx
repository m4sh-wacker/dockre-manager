...
export function RenewModal({ open, onOpenChange, container }: RenewModalProps) {
  ...
  const handleRenew = async () => {
    ...
    try {
      await renewContainer(container.id, duration, durationUnit);
      ...
      toast({
        ...
        description: `${container.serviceName} has been renewed by ${duration} ${durationUnit}.`,
      });
    } catch {
      setIsRenewing(false);
      toast({
        ...
        description: `Could not renew ${container.serviceName}. Please try again.`,
      });
    }
  };
